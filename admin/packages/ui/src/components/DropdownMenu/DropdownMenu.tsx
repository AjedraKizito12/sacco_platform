...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.SubContent
    ref={ref}
    className={cn(
      "z-[var(--z-overlay)] min-w-[8rem] overflow-hidden rounded-[var(--radius-md)] border border-[var(--border-subtle)]",
      "bg-[var(--surface-elevated)] p-1 text-[var(--text-primary)] shadow-[var(--shadow-lg)]",
...
>(({ className, sideOffset = 4, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        "z-[var(--z-overlay)] max-h-[var(--radix-dropdown-menu-content-available-height)] min-w-[8rem]",
        "overflow-y-auto overflow-x-hidden rounded-[var(--radius-md)] border border-[var(--border-subtle)]",
        "bg-[var(--surface-elevated)] p-1 text-[var(--text-primary)] shadow-[var(--shadow-md)]",
        "data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95",
        "data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 origin-[--radix-dropdown-menu-content-transform-origin]",
        className,
      )}
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Item
    ...
    className={cn(
      ...
      "text-[var(--text-body)] text-[var(--text-primary)] outline-none transition-colors",
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn(
      "px-2 py-1.5 font-semibold text-[var(--text-body)] text-[var(--text-primary)]",
      inset && "pl-8",
      className,
    )}
    {...props}
...
const DropdownMenuShortcut = ({ className, ...props }: React.HTMLAttributes<HTMLSpanElement>) => {
  return (
    <span
      className={cn(
        "ml-auto tracking-widest text-[var(--text-secondary)] text-[var(--text-small)]",
        className,
      )}
      {...props}
    />

"""Integration tests for void-invoice and cancel-subscription maker-checker paths."""
...
async def _setup_subscription_and_invoice(
...
async def test_void_invoice_creates_approval_request(
    ...
    _, _, _, inv_id = await _setup_subscription_and_invoice(factory)
    try:
        r = await client.post(
            f"/platform/billing/invoices/{inv_id}/void",
            headers={"X-Platform-Actor-ID": str(actor.id)},
            json={"reason": "voiding for test"},
        )
...
async def test_void_invoice_404_for_unknown(
    test_engine: AsyncEngine, client: AsyncClient
) -> None:
    factory = async_sessionmaker(test_engine, expire_on_commit=False)
    actor = await _create_superuser(factory)
    try:
        r = await client.post(
            f"/platform/billing/invoices/{uuid.uuid4()}/void",
            headers={"X-Platform-Actor-ID": str(actor.id)},
            json={"reason": "should 404"},
        )
...
async def test_cancel_subscription_immediate_creates_approval_request(
    ...
    factory = async_sessionmaker(test_engine, expire_on_commit=False)
    actor = await _create_superuser(factory)
    _, _, sub_id, _ = await _setup_subscription_and_invoice(factory)
    try:
        r = await client.post(
            f"/platform/billing/subscriptions/{sub_id}/cancel?mode=immediate",
            headers={"X-Platform-Actor-ID": str(actor.id)},
            json={"reason": "immediate cancel"},
        )
        assert r.status_code == 200, r.text
        body = r.json()
        assert body["status"] == "pending_approval"
...
async def test_cancel_subscription_at_period_end_marks_directly(
    test_engine: AsyncEngine, client: AsyncClient
) -> None:
    factory = async_sessionmaker(test_engine, expire_on_commit=False)
    actor = await _create_superuser(factory)
